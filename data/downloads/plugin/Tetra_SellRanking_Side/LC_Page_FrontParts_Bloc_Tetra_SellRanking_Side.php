...
class LC_Page_FrontParts_Bloc_Tetra_SellRanking_Side extends LC_Page_FrontParts_Bloc {
    ...
    public function getList($dispNumber = 0, $pageNumber = 0, $has_deleted = false)
    {
        ...
        // プラグインの設定情報を取得
        $arrPlugin = array();
        $arrPlugin = SC_Plugin_Util_Ex::getPluginByPluginCode("Tetra_SellRanking_Side");
    ...
    {
        $objQuery =& SC_Query_Ex::getSingletonInstance();
        $col = 'product_id,TRUNCATE(AVG(recommend_level),0) AS recommend_level,COUNT(*) AS recommend_count';
        $table = 'dtb_review';
        $where = '';
        $objQuery->setGroupBy('product_id');
