...
<div class="block_outer ui-section margin-section">
  <div id="plg_Tetra_SellRanking_area">
    ...
    <div class="ui-section-body">
      <div class="row">
        <!--{*
          商品取得数はデフォルトで8まで取得、変更する場合はプラグイン設定から行う
        *}-->
        <!--{foreach from=$arrBestProducts item=arrProduct name="bestseller"}-->
          ...
          <!--{* 2カラムで改行（mobileで有効） *}-->
          <!--{if $smarty.foreach.bestseller.iteration % 2 === 0}-->
            <div class="clearfix visible-xs-block"></div>
          <!--{/if}-->
          <!--{* 4カラムで改行（PC/tabletで有効） *}-->
          <!--{if $smarty.foreach.bestseller.iteration % 4 === 0}-->
